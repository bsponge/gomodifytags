...
pipeline {
    ...
    stages {
		...
		sh "git clone https://github.com/bsponge/gomodifytags"
		sh "docker cp gomodifytags cloner:/input"
            }
        }
        stage('build') {
            steps {
        ...
        stage('cleanup') {
            steps {
                ...
                sh "docker cp copier:/output/containerd-nydus-grpc ./containerd-nydus-grpc"
